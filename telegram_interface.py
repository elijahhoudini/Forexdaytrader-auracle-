...
"""
    ...
    async def status_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            status_info = {
                'running': getattr(self.trader, 'running', False),
                'paused': getattr(self.trader, 'paused', False),
                'live_mode': getattr(self.trader, 'live_mode', False),
                'pending_trades': len(getattr(self.trader, 'pending_trades', {})),
                'blacklisted_tokens': len(getattr(self.trader, 'blacklisted_tokens', set())),
                'websocket_connected': getattr(self.trader, 'websocket_client', None) is not None,
                'session_stats': getattr(self.trader, 'session_stats', {
                    'trades_executed': 0,
                    'successful_trades': 0,
                    'failed_trades': 0,
...
"""
    ...
    async def force_sell_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            try:
                ...
                await update.message.reply_text(f"❌ Error accessing positions: {e}")
    ...
    async def positions_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            positions = getattr(self.trader.auracle.trade_handler, 'open_positions', {})
...
"""
    ...
    async def performance_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            stats = getattr(self.trader, 'session_stats', {
                'trades_executed': 0,
                'successful_trades': 0,
                'failed_trades': 0,
...
"""
            ...
            trade_journal = getattr(self.trader, 'trade_journal', [])
            if len(trade_journal) > 0:
                recent_trades = trade_journal[-5:]  # Last 5 trades
...
"""
    ...
    async def blacklist_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            blacklisted_tokens = getattr(self.trader, 'blacklisted_tokens', set())
            ...
            if not context.args:
                ...
                blacklist_text = f"**🚫 Token Blacklist ({len(blacklisted_tokens)} tokens):**\n\n"
                
                for i, token in enumerate(list(blacklisted_tokens)[:20]):  # Show first 20
                    blacklist_text += f"• `{token[:8]}...{token[-8:]}`\n"
                
                if len(blacklisted_tokens) > 20:
                    blacklist_text += f"\n... and {len(blacklisted_tokens) - 20} more"
...
• Live Mode: {'🔴 YES' if getattr(self.trader, 'live_mode', False) else '🟡 NO'}
• Max Buy Amount: {getattr(config, 'MAX_BUY_AMOUNT_SOL', 0.001)} SOL
• Scan Interval: {getattr(config, 'SCAN_INTERVAL_SECONDS', 60)}s
• Max Daily Trades: {getattr(config, 'MAX_DAILY_TRADES', 50)}
• Max Open Positions: {getattr(config, 'MAX_OPEN_POSITIONS', 10)}

**🛡️ Risk Management:**
• Profit Target: {getattr(config, 'PROFIT_TARGET_PERCENTAGE', 0.20) * 100}%
• Stop Loss: {getattr(config, 'STOP_LOSS_PERCENTAGE', -0.05) * 100}%
• Min Liquidity: ${getattr(config, 'MIN_LIQUIDITY_USD', 1000):,.0f}

**🌐 Network:**
• RPC Endpoint: {getattr(config, 'SOLANA_RPC_ENDPOINT', 'https://api.mainnet-beta.solana.com')}
• WebSocket: {'✅' if getattr(self.trader, 'websocket_client', None) else '❌'}
